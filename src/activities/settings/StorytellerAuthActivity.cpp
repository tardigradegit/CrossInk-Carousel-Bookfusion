...
void StorytellerAuthActivity::onWifiSelectionComplete(const bool success) {
  if (!success) {
...
void StorytellerAuthActivity::requestCode() {
  StorytellerDeviceCodeResponse resp;
  const auto result = StorytellerSyncClient::requestDeviceCode(resp);

  if (result != StorytellerSyncClient::OK) {
    LOG_ERR("STAuth", "requestDeviceCode failed: %s", StorytellerSyncClient::errorString(result));
    RenderLock lock(*this);
    state = FAILED;
    requestUpdate(true);
    return;
...
void StorytellerAuthActivity::doPoll() {
  {
    RenderLock lock(*this);
    state = POLLING;
  }
  ...
  char tokenBuf[512] = {};
  const auto result = StorytellerSyncClient::pollForToken(deviceCode, tokenBuf, sizeof(tokenBuf));

  switch (result) {
    case StorytellerSyncClient::OK:
      ST_TOKEN_STORE.setToken(tokenBuf);
    ...
    case StorytellerSyncClient::PENDING:
    ...
    case StorytellerSyncClient::SLOW_DOWN:
      pollIntervalSec += 5;
      nextPollAt = millis() + static_cast<unsigned long>(pollIntervalSec) * 1000UL;
      {
        RenderLock lock(*this);
    ...
    case StorytellerSyncClient::NETWORK_ERROR:
      networkRetries++;
...
void StorytellerAuthActivity::onEnter() {
  Activity::onEnter();
...
void StorytellerAuthActivity::loop() {
...
void StorytellerAuthActivity::render(RenderLock&&) {
  ...
  if (state == REQUESTING_CODE || state == CONNECTING) {
    renderer.drawCenteredText(UI_10_FONT_ID, (pageHeight - lineH) / 2, tr(STR_ST_WAITING));
  ...
  if (state == WAITING_FOR_USER || state == POLLING) {
    ...
    renderer.drawCenteredText(UI_10_FONT_ID, y, tr(STR_ST_VISIT_URL), true, EpdFontFamily::BOLD);
    y += lineH + 4;

    renderer.drawCenteredText(UI_12_FONT_ID, y, verificationUri, true, EpdFontFamily::REGULAR);
  ...
  if (state == SUCCESS) {
    renderer.drawCenteredText(UI_10_FONT_ID, (pageHeight - lineH) / 2, tr(STR_ST_LINK_SUCCESS), true,
                              EpdFontFamily::BOLD);
    const auto labels = mappedInput.mapLabels(tr(STR_BACK), "", "", "");
    GUI.drawButtonHints(renderer, labels.btn1, labels.btn2, labels.btn3, labels.btn4);
    renderer.displayBuffer();
  ...
  const char* msg = tr(STR_ST_AUTH_FAILED);
